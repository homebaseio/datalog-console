...
(defn handle-devtool-message [devtool-port message _port]
  (cond
    ...
    (let [tab-id (gobj/get message "tab-id")]
      (js/console.log #js {:tab-id tab-id :message message})
    ...
    (let [tab-id      (gobj/get message "tab-id")
      ...
      (js/console.log #js {:tab-id tab-id :remote-port remote-port :message message})
      (when remote-port (.postMessage remote-port message)))))
...
(defn handle-remote-message [ch message port]
  (js/console.log "handle-remote-message" ch message port)
  (cond
    ...
    ; set icon and popup
    (gobj/getValueByKeys message "datalog-console-db-detected")
    (let [tab-id (gobj/getValueByKeys port "sender" "tab" "id")]
